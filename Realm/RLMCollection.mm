...
NSArray *RLMCollectionValueForKey(Collection& collection, NSString *key,
    ...
    if (info.rlmObjectSchema.isSwiftClass) {
        ...
        if (prop && prop.array && prop.swiftIvar) {
            Class cls = object_getIvar(accessor, prop.swiftIvar);
            for (size_t i = 0; i < count; ++i) {
                id list = [[cls alloc] init];
//                [list set_rlmArray:[[RLMManagedArray alloc] initWithList:collectionj realm:<#(RLMRealm *const __unsafe_unretained)#> parentInfo:<#(RLMClassInfo *)#> property:<#(RLMProperty *const __unsafe_unretained)#>]];
