...
- (void)testValueForCollectionOperationKeyPath
{
    ...
    XCTAssertEqualObjects([allCompanies valueForKeyPath:@"unionOfArrays.employees.name"],
                          (@[@"Joe", @"John", @"Jill", @"A", @"B", @"C", @"A"]));
    NSArray *distinct = [allCompanies valueForKeyPath:@"@distinctUnionOfArrays.employees.name"];
    NSSet *set = [NSSet setWithArray:distinct];
    XCTAssertEqual(distinct.count, set.count);
    XCTAssertEqualObjects(set, ([NSSet setWithArray:@[@"Joe", @"John", @"Jill", @"A", @"B", @"C"]]));
